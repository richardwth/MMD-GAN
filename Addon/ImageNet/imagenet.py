...
def extract_train(tar_name, target_dir, with_rec=False, num_thread=1):
    ...
    with tarfile.open(tar_name) as tar:
        ...
        for class_tar in tar:
            # extract the tar file for each class
            pbar.set_description('Extract '+class_tar.name)
            tar.extract(class_tar, target_dir)
            # get path for class.tar
            class_filename = os.path.join(target_dir, class_tar.name)
            # get folder name
            class_dir = os.path.splitext(class_filename)[0]
            # create folder with the class name
            os.mkdir(class_dir)
            # extract class.tar
            with tarfile.open(class_filename) as f:
