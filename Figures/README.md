...
In previous MMD-GAN [2], the discriminator loss, what we called attractive, is ![equation](https://latex.codecogs.com/gif.latex?\inline&space;L_D^{\text{att}}=-L_G). Below is an illustration of the effects of MMD losses on free particles (or discriminator outputs of samples if, for illustration purpose, we allow them to move freely). These GIFs extend the Figure 1 in [1].
...
In the first row, we randomly initialized the free particles, and applied <img src="https://latex.codecogs.com/gif.latex?\inline&space;L_D^{\text{att}}" title="L_D^{\text{att}}"/> and <img src="https://latex.codecogs.com/gif.latex?\inline&space;L_D^{\text{rep}}" title="L_D^{\text{rep}}"/> for 600 steps. The velocity of each particle is <img src="https://latex.codecogs.com/gif.latex?\inline&space;-0.01\nabla{L_D}" title="vD"/>. In the second row, we obtained the particle position at the 450th step of the first row and applied |<img src="https://latex.codecogs.com/gif.latex?\inline&space;L_G" title="L_G"/> for another 600 steps with velocity <img src="https://latex.codecogs.com/gif.latex?\inline&space;-0.01\nabla{L_G}" title="vG"/>. The blue and orange arrows stand for the gradients from attractive and repulsive components of MMD losses respectively. In summary, these GIF indicates how MMD losses would affect the free particles. Of course, the actual case of MMD-GAN would be much more complex as we update the model parameters instead of output scores directly and G and D are constantly changing at each step. 
